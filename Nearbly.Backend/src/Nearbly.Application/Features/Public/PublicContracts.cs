...
public sealed record PublicStoreResponse(Guid Id, string Name, string Slug, string PublicCode, string? Description, string? LogoUrl, string? PrimaryColor, string? SecondaryColor, IReadOnlyList<PublicLinkResponse> Links, IReadOnlyList<PublicTabResponse> Tabs);
public sealed record RegisterPageViewRequest(TrafficSource? Source = null);

public interface IPublicService
{
    Task<PublicStoreResponse?> GetStoreAsync(string identifier, CancellationToken cancellationToken);
    Task RegisterViewAsync(string identifier, RegisterPageViewRequest request, CancellationToken cancellationToken);
